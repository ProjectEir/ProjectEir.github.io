      ...
      if (d.height == 0) {
        ...
        ajax(url).then(function(result) {
          ...
          d3.csvParse(myData, function(error, data) {
            ...
            d3.selectAll(".axis.Rank .tick text").style("fill", color);
            ...
            function brush() {
              ...
              ctx.clearRect(0, 0, width, height);
              ctx.globalAlpha = d3.min([
                0.85 / Math.pow(selected.length, 0.3),
                1
              ]);
              render(selected);
        ...
        function makeUrl(disease) {
          ...
          return (
            ...
            disease +
            "%0D%0A&fields=BriefTitle%2C+Condition%2C+Phase%2C+EnrollmentCount%2C+StartDate%2C+CompletionDate%2C+LastUpdatePostDate%2C+OutcomeMeasureAnticipatedPostingDate%2C+ResultsFirstPostDate%2C+ResultsFirstSubmitDate&min_rnk=1&max_rnk=1000&fmt=csv"
          );
        }
        ...
        function makeDimensions() {
          ...
          if (
            document.getElementById("OutcomeMeasureAnticipatedPostingDate")
              .checked
          ) {
            dim = dim.concat({
